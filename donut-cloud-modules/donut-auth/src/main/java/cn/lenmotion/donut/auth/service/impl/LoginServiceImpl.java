...
import cn.lenmotion.donut.common.core.constants.ConfigConstants;
import cn.lenmotion.donut.common.core.constants.RedisConstants;
...
import cn.lenmotion.donut.common.redis.template.JacksonRedisTemplate;
import cn.lenmotion.donut.system.client.SysConfigFeignClient;
import cn.lenmotion.donut.system.client.SysLoginLogFeignClient;
import cn.lenmotion.donut.system.client.SysPermissionFeignClient;
import cn.lenmotion.donut.system.client.SysUserFeignClient;
...
public class LoginServiceImpl implements LoginService {
    ...
    private final SysUserFeignClient userFeignClient;
    private final SysConfigFeignClient configFeignClient;
    private final SysPermissionFeignClient permissionFeignClient;
    private final SysLoginLogFeignClient loginLogFeignClient;
    ...
    public void login(LoginBody loginBody, HttpServletRequest request) {
        ...
        try {
            ...
            // 验证验证码是否正确
            if (configFeignClient.getConfigBoolValue(ConfigConstants.CAPTCHA_ON_OFF)) {
            ...
            // 查询用户
            var user = userFeignClient.getByUsername(loginBody.getUsername());
            // 判断用户存在与否
...
//            noticeRemoteService.closeNoticeWebsocket(StpUtil.getTokenInfo());
    ...
    private void loginLog(LoginBody loginBody, HttpServletRequest request, Exception e) {
        ...
        taskExecutor.execute(() -> {
            try {
                ...
                loginLogFeignClient.saveLoginLog(loginLog);
            } catch (Exception e1) {
     ...
     * @param request
     */
    private void handleLoginSuccess(SysUserDTO user, HttpServletRequest request) {
        SysUser sysUser = new SysUser();
        sysUser.setId(user.getId());
        sysUser.setLoginIp(IpUtils.getIpAddr(request));
        sysUser.setLoginDate(LocalDateTime.now());
        userFeignClient.updateUser(sysUser);

        LoginInfo loginInfo = new LoginInfo();
        loginInfo.setUserId(user.getId());
        loginInfo.setUsername(user.getUsername());
        loginInfo.setDeptAncestors(permissionFeignClient.getDeptAncestors(user.getId()));
        loginInfo.setRoleDataScopes(permissionFeignClient.getRoleDataScope(user.getId()));
